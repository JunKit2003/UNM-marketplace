...
module.exports = async function uploadListingDetails(req, res) {
  ...
  console.log("Received data from the request:", { title, description, price, category });

  // Insert the new listing into the database
  const insertQuery = 'INSERT INTO listing (title, description, price, category) VALUES (?, ?, ?, ?)';
  await db.query(insertQuery, [title, description, price, category], (insertErr, insertResult) => {
    if (insertErr) {
