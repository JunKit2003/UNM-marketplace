...
module.exports = async function uploadListingDetails(req, res) {
  ...
  const { title, description, price, category, PostedBy } = req.body;

  console.log("Received data from the request:", { title, description, price, category, PostedBy });

  // Insert the new listing into the database
  const insertQuery = 'INSERT INTO listing (title, description, price, category, PostedBy) VALUES (?, ?, ?, ?, ?)';
  await db.query(insertQuery, [title, description, price, category, PostedBy], (insertErr, insertResult) => {
    if (insertErr) {
